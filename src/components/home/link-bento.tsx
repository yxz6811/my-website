    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:grid-rows-2 md:gap-4">
      {product && (
        <motion.a
          ...
          className={`group relative col-span-1 flex min-h-[200px] flex-col justify-between overflow-hidden rounded-3xl border border-white/12 bg-gradient-to-br ${variantAccent[product.variant]} p-6 text-left md:col-span-2 md:row-span-2 ${variantRing[product.variant]}`}
          whileHover={reduce ? undefined : { y: -4, transition: { duration: 0.2 } }}
          whileTap={reduce ? undefined : { scale: 0.99 }}
        >
          <div className="absolute right-0 top-0 h-40 w-40 translate-x-10 -translate-y-10 rounded-full bg-white/5 blur-2xl transition group-hover:bg-white/10" />
          <div className="relative flex items-start justify-between gap-4">
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{product.label}</p>
              </div>
            </div>
          ...
          <p className="relative mt-4 max-w-md text-sm leading-relaxed text-zinc-200/90">{product.blurb}</p>
        </motion.a>
      )}
