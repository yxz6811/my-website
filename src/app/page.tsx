...
/**
 * 首页：结构与样式；文案来自 `src/content/site-profile.ts`。
 */
    ...
    <main className="relative min-h-screen overflow-hidden bg-zinc-950 text-zinc-100">
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-zinc-950/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl flex-col gap-3 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm font-semibold tracking-tight text-white">杨曦哲</p>
          <nav className="flex flex-wrap gap-2 text-sm">
            {navItems.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-zinc-200 transition hover:border-white/25 hover:bg-white/10"
              >
                {item.label}
              </a>
            ))}
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col gap-14 px-6 py-14">
        <section
          id="about"
          className="scroll-mt-28 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-md md:p-12"
        >
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-black/20 p-6 text-sm leading-7 text-zinc-300 md:text-base">
            {siteProfile.mediaLine}
          </div>
              ...
              return (
                <a
                  ...
                  {...(isHttp
                    ...
                    : {})}
                  className="inline-flex h-11 items-center justify-center rounded-full border border-white/20 bg-white/5 px-5 text-sm font-semibold text-zinc-100 transition hover:bg-white/10"
                >
                  {link.label}
                </a>
              );
        ...
        >
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-zinc-300 md:text-lg">{siteProfile.abilities}</p>
        </section>
        ...
        >
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-2">
            ...
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-wider text-violet-200/90">
                正在加强
              </h3>
              <ul className="mt-4 space-y-3 text-sm leading-7 text-zinc-300 md:text-base">
